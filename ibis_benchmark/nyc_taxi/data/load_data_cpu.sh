...
OMNISCIDB_CPU_PARAMS=" --db omnisci --user admin --passwd HyperInteractive  --port 6274"

# CREATE TABLE
echo $(echo "$CMD_CREATE_SCHEMA" | omnisql $OMNISCIDB_CPU_PARAMS)
echo $(echo "$CMD_SHOW_TABLES" | omnisql $OMNISCIDB_CPU_PARAMS)

# LOAD DATA
echo $CMD_COPY
echo $(echo "$CMD_COPY" | omnisql $OMNISCIDB_CPU_PARAMS)
echo $(echo "$CMD_SELECT_TABLE" | omnisql $OMNISCIDB_CPU_PARAMS)
